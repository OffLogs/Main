...
namespace OffLogs.Api.Tests.Integration.Core.Service
{
    public interface IDataSeederService
    {
        ...
        Task<IList<LogEntity>> CreateLogsAsync(long applicationId, LogLevel level, int counter = 1);
        Task<LogEntity> MakeLogAsync(ApplicationEntity application, LogLevel level);
        Task<IList<ApplicationEntity>> CreateApplicationsAsync(UserEntity user, int counter = 1);
    }
}
